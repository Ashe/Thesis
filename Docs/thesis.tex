...
\begin{itemize}
\item \textbf{The speed of execution:} Unless significantly slow to the point where the game AI is rendered impractical, the time it takes for the algorithm to finish processing will not be recorded.
\end{itemize}
...
The game needs to contain a variety large enough to test both player and AI. Strategy games can easily give simple mechanics depth. The only real requirement in terms of gameplay is that players need to be able to perform more than one action per turn --- \emph{TODO: write about tic-tac-toe making this requirement?}
...
\item \textbf{Implementation of a `dumb' team controller:}
...
\item \textbf{Ability for A* to play:}

\item \textbf{A method of stepping through the AI's turn:}
